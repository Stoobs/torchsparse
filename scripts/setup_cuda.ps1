...
# Define CUDA version to URL mapping
$cudaVersionToURL = @{
    '12.9.0' = 'https://developer.download.nvidia.com/compute/cuda/12.9.0/local_installers/cuda_12.9.0_576.02_windows.exe'
...
$downloadUrl = $cudaVersionToURL[$CUDA_VERSION_FULL]
if (-not $downloadUrl) {
    Write-Output "Unsupported CUDA version specified: $CUDA_VERSION_FULL"
    Write-Output "Supported versions:"
    $cudaVersionToURL.Keys | Sort-Object -Descending | ForEach-Object {
        Write-Output "  - $_"
    }
    exit 1
}
